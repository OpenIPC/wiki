...
```
### Первый запуск
Не прерываем загрузку бута и наблюдаем лог запуска системы. Если всё прошло штатно и в использованной сборке ничего не отломано, то через несколько секунд увидим приглашение входа. Логинимся под пользователем root. У новой камеры пароля ещё нет, поэтому она сразу предложит его задать -- шелл выдаётся только после этого. Затем вводим команду ifconfig eth0, чтобы увидеть полученный IP-адрес.
Веб-интерфейс доступен на порту 80. Пароля у новой камеры нет, и пока он не задан, камера ничего не транслирует, поэтому первым делом она предложит его придумать. Это пароль пользователя root -- он же используется при входе в консоль через UART или SSH, а также для RTSP. ONVIF принимает его от клиентов с HTTP Basic или WSSE PasswordText; клиентам, работающим по дайджесту, нужен отдельно заданный `onvif.password`.
...
- rtsp://root:password@ip-address:554/stream=0 — первый поток
- rtsp://root:password@ip-address:554/stream=1 — второй поток
