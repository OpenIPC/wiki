...
перезагрузится — вместе с оверлеем стирается и пароль root, поэтому после этого
камеру нужно принять заново, см.
[Сброс к заводским настройкам и «непринятая» камера](first-boot.md).
...
Для настройки прошивки откройте в браузере веб-интерфейс вашей
камеры. У новой камеры пароля нет, и пока он не задан, она ничего не
транслирует, поэтому первым делом она предложит вам придумать пароль. Это
пароль пользователя __root__ -- его же проверяют веб-интерфейс, SSH и RTSP.
Только после этого камера пустит вас к рычагам управления.

ONVIF принимает этот же пароль от клиентов, которые присылают HTTP Basic или
WSSE PasswordText. Клиентам, которые авторизуются дайджестом (WSSE
PasswordDigest -- его большинство ONVIF-клиентов пробует первым -- или HTTP
Digest), он не подойдёт: дайджест считается от пароля в открытом виде, а в
системной базе хранится только необратимый хэш. Для таких клиентов задайте
отдельно `onvif.password` и `onvif.username` в конфигурации Majestic.
