# OpenIPC Wiki
[Оглавление](../README.md)

Ростелеком IPC-HDW1230SP v3
--------------

## Платформа
- Процессор SigmaStar SSC335DE
- Память NOR 16 Mb
- Сенсор GalaxyCore GC2053

## Дампы 
Качаем с GitHub архивы с загрузчиком и образами ядра и файловой системы для имеющегося процессора.
- [u-boot-ssc335de-nor.bin](https://github.com/OpenIPC/firmware/releases/download/latest/u-boot-ssc335de-nor.bin)
- [openipc.ssc335de-nor-lite.tgz](https://github.com/OpenIPC/firmware/releases/download/latest/openipc.ssc335de-nor-lite.tgz)

Загрузчик кладём в папку tftp-сервера как есть, а образы ядра и файловой системы нужно предварительно распаковать. 
Для прошивки понадобится подключиться через UART, что соответственно, потребует адаптер USB-TTL, коннектор к нему и программу-терминал.

### Подключение
- Подключаем коннектор к камере и адаптеру.
- Подключаем адаптер к компьютеру - появится виртуальный COM-порт. Исхожу из того, что драйверы уже установлены. 
- Запускаем терминал. 
- Подаём питание на камеру и, как только побежит лог загрузки, нажимаем клавишу Enter для остановки запуска и попадания в загрузчик.

### Прошивка
Первым делом шьём загрузчик
```
setenv ipaddr 192.168.1.10; setenv serverip 192.168.1.254
mw.b 0x21000000 0xff 0x50000
tftpboot 0x21000000 u-boot-ssc335de-nor.bin
sf probe 0
sf erase 0x0 0x50000; sf write 0x21000000 0x0 0x50000
reset
```
Если всё прошло успешно, после перезапуска системы нужно будет снова остановить запуск для попадания уже в загрузчик OpenIPC.
Шьём ядро и файловую систиему с помощью макросов.
```
setenv ipaddr 192.168.1.10; setenv serverip 192.168.1.254
run uknor
run urnor
reset
```
### Первый запуск
Не прерываем загрузку и наблюдаем лог запуска системы. Если всё прошло штатно, то через несколько секунд увидим приглашение входа.
Логинимся под пользователем root. У новой камеры пароля ещё нет, поэтому она сразу предложит его задать, и только после этого выдаст шелл. Дальше приступаем к конфигурированию. Сенсор определяется автоматчески а натсроить нужно
только переключение режима день/ночь и микрофон.
```
curl http://localhost/api/v1/config --data-binary @- <<'EOF'
{
  "nightMode": {
    "irCutPin1": 79,
    "irCutPin2": 78,
    "backlightPin": 52,
    "lightSensorPin": 80,
    "lightSensorInvert": true,
    "lightMonitor": true,
    "colorToGray": true
  },
  "audio": {
    "enabled": true,
    "codec": "aac",
    "srate": 48000,
    "inputChannel": 1
  }
}
EOF
reboot
```
После перезапуска можно увидеть в консоли, какой адрес получит камера и зайти по нему в браузере. Если не получилось увидеть, то нужно снова залогиниться и ввести команду ifconfig eth0.
В веб-интерфейс входим под тем же пользователем root и тем паролем, который задали при первом запуске. Если камера ещё не настроена, веб-интерфейс сам предложит придумать пароль -- он же станет паролем root при входе в консоль через UART или SSH.
Бужет предложено задать и валидный MAC-адрес, после чего изменится и выдаваемый роутером IP-адрес.

Можно пользоваться!
