...
- распакуйте архив и поместите содержимое папки OpenIPC в корень SD/MMC карты
...
Готовые файлы для безразборочной прошивки и базовая техподдержка доступны в
открытой [Telegram](https://openipc.org/our-channels) группе нашего проекта.
...
Исходные коды проекта, для тех кто хочет собрать прошивку полностью [самостоятельно](https://github.com/OpenIPC).
...
возможен на порту 22 под пользователем root. У новой камеры пароля нет, и пока он не задан,
она ничего не транслирует: и веб-интерфейс, и первый интерактивный вход по SSH сначала
предложат придумать пароль.
...
Зайдите на камеру по протоколу SSH через программу Putty (логин root, порт 22, пароль --
тот, который вы задали при первой настройке камеры) и
выполните команду:
...
Камера предложит придумать пароль -- это пароль пользователя root (не забудьте его записать!).
...
буквами). MAC написан на титульной странице веб-интерфейса (порт 80).
...
#### Что делать если не показывает видео в сервисе ipeye.ru ?

Иногда бывает что при прошивке пользователи не следуют инструкциям и путают порядок
действий или слишком спешат. В этом случае после прошивки на устройстве остаются
неправильные настройки и присутствует интерфейс Ethernet, которого реально на устройстве
нет. Для проверки есть-ли проблема, выполните в UART- SSH- или WEB-консоли команду
`ip a | grep eth0`. Если вы видите наличие этого интерфейса в ответе, то увас есть
проблема и её можно решить при помощи SD карты.
...
```
Отключите питание у камеры, вставьте SD карту, включите питание и подождите две минуты.
После этого еще раз отключите и включите питание, дождитесь загрузки устройства и проверьте
работу. Для завершающего контроля выполнить команду `ip a | grep eth0` - если все действия
выполнены правильно то никакого отображения интерфейча eth0 быть не должно.
