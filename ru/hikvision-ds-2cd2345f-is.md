...
## Веб-интерфейс
Веб-интерфейс доступен на порту 80. Пароля у новой камеры нет, и пока он не задан, камера ничего не транслирует, поэтому первым делом она предложит его придумать. Это пароль пользователя root -- он же используется при входе в консоль через UART или SSH, а также для RTSP и ONVIF.
