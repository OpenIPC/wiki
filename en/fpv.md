...
* After booting the system, verify that it has obtained an IP address via Ethernet (it can be accessed via SSH as root@<IP address of the camera>; a camera that has not been set up yet has no password and asks you to choose one before handing over a shell). Using a modern operating system and the name `openipc.local` you can find the camera within the local network without knowing its IP address. 
